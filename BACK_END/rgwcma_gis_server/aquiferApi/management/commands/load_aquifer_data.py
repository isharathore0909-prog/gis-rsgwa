...
class Command(BaseCommand):
    help = 'Load aquifer groundwater level data from Rajasthan_1.1_WL_Data_cleaned.xlsx'

    def handle(self, *args, **options):
        file_path = r'D:\GIS_RSGWA_ANALYSIS\BACK_END\Rajasthan_1.1_WL_Data_cleaned.xlsx'
        log_file = 'aquifer_import_log.txt'
        ...
        with open(log_file, 'w') as f:
            f.write(f"Starting aquifer data import at {datetime.datetime.now()}\n")
        ...
        col_map = {
            ...
            'village': find_col(['village name', 'village', 'location']),
            'latitude': find_col(['latitude', 'lat']),
            'longitude': find_col(['longitude', 'long', 'lon']),
            'well_id': find_col(['well id', 'well_id', 'station id', 'station_id', 'id']),
            'well_depth': find_col(['well depth', 'depth', 'total depth']),
            'aquifer': find_col(['aquifer', 'aquifer type']),
        }
        
        # Map year columns (pre and post for 2015-2024)
        ...
        for year in years:
            ...
            col_map[f'pre_{year}'] = find_col(pre_patterns)
            col_map[f'pst_{year}'] = find_col(pst_patterns)

        log(f"Column Mapping: {col_map}")

        # Check critical columns
        if not col_map['district'] or not col_map['village']:
            ...
            with transaction.atomic():
                for index, row in chunk.iterrows():
                    try:
                        ...
                        # Extract well data
                        well_id = str(row[col_map['well_id']]).strip() if col_map['well_id'] and pd.notna(row[col_map['well_id']]) else f"AQ-{v_name}-{index}"
                        ...
                        aquifer_val = str(row[col_map['aquifer']]).strip() if col_map['aquifer'] and pd.notna(row[col_map['aquifer']]) else None

                        # Extract measurements for all years
                        def safe_float(col_key):
                            if not col_map.get(col_key):
                                return None
                            try:
                        ...
                        # Build defaults dict with all year measurements
                        defaults = {
                            'village': v_obj,
                            'latitude': lat_val,
                            'longitude': lon_val,
                        ...
                        # Add all year measurements
                        for year in range(2015, 2025):
                            defaults[f'pre_{year}'] = safe_float(f'pre_{year}')
                            defaults[f'pst_{year}'] = safe_float(f'pst_{year}')

                        # Create or update aquifer record
                        aq_obj, created = AquiferData.objects.update_or_create(
                            well_id=well_id,
                            defaults=defaults
                        )
